...
public class GhostPokemon extends Pokemon{
    ...
    public String useMove() {
        return("Ghost type Pokemon can use Shadow Ball as a default move.");
    }
    ...
    public String getTypeWeakness() {
        ...
        commonWeaknesses.retainAll(Type.GHOST.getWeaknesses()); // checks for common weaknesses between type two and type one
        // it will get rid of all non-shared weaknesses

        List<Type> conflictingWeaknesses = new ArrayList<>(weaknesses);
        conflictingWeaknesses.retainAll(Type.GHOST.getResistances()); // if type one is resistant to a weakness of the secondary type it cancels out

        List<Type> secondaryConflictingWeaknesses = new ArrayList<>(Type.GHOST.getWeaknesses());
        ...
        for(Type type : Type.GHOST.getWeaknesses()){
        ...
        if(typeTwo != Type.NONE){ //make sure there is a secondary type in the first place
            ...
            toReturn.append("2x Weaknesses: DARK,GHOST");
        }
