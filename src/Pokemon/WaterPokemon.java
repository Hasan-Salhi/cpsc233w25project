...
public class WaterPokemon extends Pokemon {
    public WaterPokemon(String name, int hp, int attack, Type typeOne, Type typeTwo){
        super(name, hp, attack, typeOne, typeTwo);
    }

    @Override
    public String useMove() {
        return("Water type Pokemon can use Hydro Pump as a default move.");
    }
    ...
    public String getTypeWeakness() {
        ...
        commonWeaknesses.retainAll(Type.WATER.getWeaknesses()); // checks for common weaknesses between type two and type one
        // it will get rid of all non-shared weaknesses

        List<Type> conflictingWeaknesses = new ArrayList<>(weaknesses);
        conflictingWeaknesses.retainAll(Type.WATER.getResistances()); // if type one is resistant to a weakness of the secondary type it cancels out

        List<Type> secondaryConflictingWeaknesses = new ArrayList<>(Type.WATER.getWeaknesses());
        ...
        for(Type type : Type.WATER.getWeaknesses()){
        ...
        if(typeTwo != Type.NONE){ //make sure there is a secondary type in the first place
            ...
            toReturn.append("2x Weaknesses: ELECTRIC, GRASS");
        }
    ...
    public String typePassive() {
        return "As a Water type Pokemon, the move 'Freeze Dry' is super effective against this Pokemon.";
    }
}
