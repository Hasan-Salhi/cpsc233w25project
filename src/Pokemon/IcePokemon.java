...
public class IcePokemon extends Pokemon{
    ...
    public String useMove() {
        return("Ice type Pokemon can use Icicle Crash as a default move.");
    }
    ...
    public String getTypeWeakness() {
        ...
        commonWeaknesses.retainAll(Type.ICE.getWeaknesses()); // checks for common weaknesses between type two and type one
        // it will get rid of all non-shared weaknesses

        List<Type> conflictingWeaknesses = new ArrayList<>(weaknesses);
        conflictingWeaknesses.retainAll(Type.ICE.getResistances()); // if type one is resistant to a weakness of the secondary type it cancels out

        List<Type> secondaryConflictingWeaknesses = new ArrayList<>(Type.ICE.getWeaknesses());
        ...
        for(Type type : Type.ICE.getWeaknesses()){
        ...
        if(typeTwo != Type.NONE){ //make sure there is a secondary type in the first place
            ...
            toReturn.append("2x Weaknesses: FIGHTING,FIRE,ROCK,STEEL");
        }
    ...
    public String typePassive() {
        return "As an Ice type Pokemon this Pokemon is immune to the frozen status.";
    }
}
