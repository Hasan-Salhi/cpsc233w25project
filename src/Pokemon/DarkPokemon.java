...
public class DarkPokemon extends Pokemon{
    public DarkPokemon(String name, int hp, int attack, Type typeOne, Type typeTwo){
        super(name, hp, attack, typeOne, typeTwo);
    }

    @Override
    public String useMove() {
        return("Dark type Pokemon can use Nasty Plot as a default move.");
    }
    ...
    public String getTypeWeakness() {
        ...
        commonWeaknesses.retainAll(Type.DARK.getWeaknesses()); // checks for common weaknesses between type two and type one
        // it will get rid of all non-shared weaknesses

        List<Type> conflictingWeaknesses = new ArrayList<>(weaknesses);
        conflictingWeaknesses.retainAll(Type.DARK.getResistances()); // if type one is resistant to a weakness of the secondary type it cancels out

        List<Type> secondaryConflictingWeaknesses = new ArrayList<>(Type.DARK.getWeaknesses());
        ...
        for(Type type : Type.DARK.getWeaknesses()){
        ...
        if(typeTwo != Type.NONE){ //make sure there is a secondary type in the first place
            ...
            toReturn.append("2x Weaknesses: BUG,FAIRY,FIGHTING");
        }
    ...
    public String typePassive() {
        return "As a Dark type Pokemon this Pokemon is immune to status moves used by opposing Pokemon with the 'Prankster' ability.";
    }
}
