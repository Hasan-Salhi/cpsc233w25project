...
public class FightingPokemon extends Pokemon{
    public FightingPokemon(String name, int hp, int attack, Type typeOne, Type typeTwo){
        super(name, hp, attack, typeOne, typeTwo);
    }

    @Override
    public String useMove() {
        return("Fighting type Pokemon can use Close Combat as a default move.");
    }
    ...
    public String getTypeWeakness() {
        ...
        commonWeaknesses.retainAll(Type.FIGHTING.getWeaknesses()); // checks for common weaknesses between type two and type one
        // it will get rid of all non-shared weaknesses

        List<Type> conflictingWeaknesses = new ArrayList<>(weaknesses);
        conflictingWeaknesses.retainAll(Type.FIGHTING.getResistances()); // if type one is resistant to a weakness of the secondary type it cancels out

        List<Type> secondaryConflictingWeaknesses = new ArrayList<>(Type.FIGHTING.getWeaknesses());
        ...
        for(Type type : Type.FIGHTING.getWeaknesses()){
        ...
        if(typeTwo != Type.NONE){ //make sure there is a secondary type in the first place
            ...
            toReturn.append("2x Weaknesses: FAIRY,FLYING,PSYCHIC");
        }
    ...
    public String typePassive() {
        return "The Fighting type unfortunately does not have any passives.";
    }
}
