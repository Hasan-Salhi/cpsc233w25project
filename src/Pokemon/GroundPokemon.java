...
public class GroundPokemon extends Pokemon{
    ...
    public String useMove() {
        return("Ground type Pokemon can use Earthquake as a default move.");
    }
    ...
    public String getTypeWeakness() {
        ...
        commonWeaknesses.retainAll(Type.GROUND.getWeaknesses()); // checks for common weaknesses between type two and type one
        // it will get rid of all non-shared weaknesses

        List<Type> conflictingWeaknesses = new ArrayList<>(weaknesses);
        conflictingWeaknesses.retainAll(Type.GROUND.getResistances()); // if type one is resistant to a weakness of the secondary type it cancels out

        List<Type> secondaryConflictingWeaknesses = new ArrayList<>(Type.GROUND.getWeaknesses());
        ...
        for(Type type : Type.GROUND.getWeaknesses()){
        ...
        if(typeTwo != Type.NONE){ //make sure there is a secondary type in the first place
            ...
            toReturn.append("2x Weaknesses: GRASS,ICE,WATER");
        }
