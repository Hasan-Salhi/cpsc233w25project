...
public class FairyPokemon extends Pokemon{
    ...
    public String useMove() {
        return("Fairy type Pokemon can use Moonblast as a default move.");
    }
    ...
    public String getTypeWeakness() {
        ...
        commonWeaknesses.retainAll(Type.FAIRY.getWeaknesses()); // checks for common weaknesses between type two and type one
        // it will get rid of all non-shared weaknesses

        List<Type> conflictingWeaknesses = new ArrayList<>(weaknesses);
        conflictingWeaknesses.retainAll(Type.FAIRY.getResistances()); // if type one is resistant to a weakness of the secondary type it cancels out

        List<Type> secondaryConflictingWeaknesses = new ArrayList<>(Type.FAIRY.getWeaknesses());
        ...
        for(Type type : Type.FAIRY.getWeaknesses()){
        ...
        if(typeTwo != Type.NONE){ //make sure there is a secondary type in the first place
            ...
            toReturn.append("2x Weaknesses: POISON,STEEL");
        }
    ...
    public String typePassive() {
        return "The Fairy type unfortunately does not have any passives.";
    }
}
