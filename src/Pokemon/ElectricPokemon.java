...
public class ElectricPokemon extends Pokemon{
    public ElectricPokemon(String name, int hp, int attack, Type typeOne, Type typeTwo){
        super(name, hp, attack, typeOne, typeTwo);
    }

    @Override
    public String useMove() {
        return("Electric type Pokemon can use Thunder as a default move.");
    }
    ...
    public String getTypeWeakness() {
        ...
        commonWeaknesses.retainAll(Type.ELECTRIC.getWeaknesses()); // checks for common weaknesses between type two and type one
        // it will get rid of all non-shared weaknesses

        List<Type> conflictingWeaknesses = new ArrayList<>(weaknesses);
        conflictingWeaknesses.retainAll(Type.ELECTRIC.getResistances()); // if type one is resistant to a weakness of the secondary type it cancels out

        List<Type> secondaryConflictingWeaknesses = new ArrayList<>(Type.ELECTRIC.getWeaknesses());
        ...
        for(Type type : Type.ELECTRIC.getWeaknesses()){
        ...
        if(typeTwo != Type.NONE){ //make sure there is a secondary type in the first place
            ...
            toReturn.append("2x Weaknesses: GROUND");
        }
    ...
    public String typePassive() {
        return "As an Electric type Pokemon this Pokemon is immune to paralysis.";
    }
}
